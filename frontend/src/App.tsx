...
function musicTelem(s: MusicState): TelemetryT {
  return {
    ...
    centroid: s.centroid, color: s.color, family: s.family, mode: s.mode,
...
export default function App() {
  ...
  const pushTelem = useCallback((t: TelemetryT) => {
    setTelem(t);
    hist.current.push({ level: t.level, bright: t.brightness / 100, color: t.color, centroid: t.centroid });
